...
namespace MMI_SP
{
    ...
    class MenuMMI
    {
        ...
        // Sub menus
     public   UIMenuItem _itemInsure;
      public  UIMenu _submenuRecover;
      public  UIMenu _submenuStolen;
      public  UIMenu _submenuCancel;
       public UIMenu _submenuPlate;
       public UIMenu _submenuBring;

        // Guard to avoid subscribing multiple times
        private bool _bringMenuOpenSubscribed = false;
                    ...
                    _submenuRecover.OnItemSelect += (sender, item, index) =>
                    {
                        if (item == recoverVehicle)
                        {
                            if (SE.Player.AddCashToPlayer(-1 * cost))
                            {
                                ...
                                //recoverVehicle = new UIMenuItem(T.GetString("Empty"), T.GetString("RecoverVehicleItemEmptyDesc")) { Enabled = false };
                                //_submenuRecover.AddItem(recoverVehicle);
        ...
        private void CreateMenuBring(UIMenu menu)
        {
            ...
            if (!_bringMenuOpenSubscribed)
            {
                _submenuBring.OnMenuOpen += SubmenuBring_OnMenuOpen;
                _bringMenuOpenSubscribed = true;
            }
        }
        ...
        private void SubmenuBring_OnMenuOpen(UIMenu sender)
        {
            if (sender != _submenuBring) return;

            RefreshMenuIndex(_submenuBring, T.GetString("BringVehicleItemEmptyDesc"));
            RebuildMenuBring();
            _menuPool.RefreshIndex();

            if (_submenuBring != null)
            {
                if (_submenuBring.MenuItems.Count > 0)
                    _submenuBring.CurrentSelection = 0;
                _submenuBring.UpdateScaleform();
            }
        }
